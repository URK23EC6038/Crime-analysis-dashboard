                ...
                if not df_logs.empty:
                    ...
                    for district in summary.index:
                        if district in df_final['District'].values:
                            if 'Theft' in summary.columns: df_final.loc[df_final['District'] == district, 'Total_Crime_Count_2022'] += summary.loc[district, 'Theft']
                            ...
                            if 'Accident' in summary.columns: df_final.loc[df_final['District'] == district, 'Road_Accidents'] += summary.loc[district, 'Accident']
                            if 'Harassment' in summary.columns: df_final.loc[df_final['District'] == district, 'Harassment'] += summary.loc[district, 'Harassment']
                            if 'Suicide' in summary.columns: df_final.loc[df_final['District'] == district, 'Suicides'] += summary.loc[district, 'Suicide']
            except: pass

        df_final['Crime_Rate_2022'] = df_final['Total_Crime_Count_2022'] / df_final['Population']
        df_final['Severity_Score'] = ((df_final['Murder']*10) + (df_final['Harassment']*5) + (df_final['Road_Accidents']*2) + (df_final['Total_Crime_Count_2022']*0.5))
...
def append_new_case(data_folder, district, crime_type, description, severity):
    try:
        ...
        new_row = {"Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "District": district, "Crime_Type": crime_type, "Description": description, "Severity": severity}
