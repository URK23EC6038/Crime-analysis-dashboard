...
app.state.df = None; app.state.geojson = None; app.state.analysis_results = {}

def convert_to_safe_json(obj):
    if isinstance(obj, (np.integer, int)): return int(obj)
    elif isinstance(obj, (np.floating, float)): return 0.0 if np.isnan(obj) or np.isinf(obj) else float(obj)
...
def startup_event(): refresh_data()
...
def run_ai_engine():
    ...
    try:
        ...
        high = df.loc[df['Severity_Score'].idxmax()]
        safe = df.loc[df['Severity_Score'].idxmin()]
        app.state.analysis_results['intel_brief'] = {
            "critical_zone": high['District'], "critical_score": round(high['Severity_Score'], 2),
            "safe_zone": safe['District'], "total_incidents": int(df['Total_Crime_Count_2022'].sum()),
            "alert_level": "RED" if df['Severity_Score'].mean() > 50 else "AMBER"
        }
    except Exception as e: print(f"AI Engine Error: {e}")
...
def get_intel_feed():
    ...
    return convert_to_safe_json({
        "brief": results.get('intel_brief', {}),
        "hotspots": results.get('clusters', []),
        "anomalies": results.get('anomalies', []),
        "predictive_drivers": results.get('feature_importance', []),
        "chart_data": {"districts": df['District'].tolist(), "composition": {
            "suicides": df['Suicides'].sum(), "accidents": df['Road_Accidents'].sum(),
            "murders": df['Murder'].sum(), "harassment": df['Harassment'].sum()
        }}
    })

@app.get("/api/model-comparison")
def get_model_comparison():
    if app.state.df is None: return []
    return ml_models.compare_models(app.state.df)

@app.get("/api/geo-layers")
def get_geo_layers():
    if not app.state.geojson or app.state.df is None: return convert_to_safe_json(app.state.geojson) if app.state.geojson else {}
    ...
    for feature in geojson['features']:
        ...
        for key in ['dtname', 'DISTRICT', 'NAME', 'Name', 'district', 'District']:
            if key in props: d_name = props[key]; break
        if d_name: props['DISTRICT'] = d_name
        else: props['DISTRICT'] = "Unknown"
        
        if d_name:
            match = None
            if d_name in df.index: match = df.loc[d_name]
            else:
                for idx in df.index:
                    if str(idx).lower().strip() == str(d_name).lower().strip(): match = df.loc[idx]; break
...
async def add_case_record(district: str = Body(...), crime_type: str = Body(...), description: str = Body(...), severity: str = Body(...)):
    if append_new_case(DATA_FOLDER, district, crime_type, description, severity):
        refresh_data(); return {"status": "success"}
...
if os.path.exists(os.path.join(FRONTEND_FOLDER, "js")): app.mount("/js", StaticFiles(directory=os.path.join(FRONTEND_FOLDER, "js")), name="js")
@app.get("/{full_path:path}")
async def serve_frontend(full_path: str):
    file_path = os.path.join(FRONTEND_FOLDER, full_path)
    return FileResponse(file_path) if os.path.isfile(file_path) else FileResponse(os.path.join(FRONTEND_FOLDER, "index.html"))
