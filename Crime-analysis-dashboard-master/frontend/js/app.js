...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    initMap(); initSparklines();
    await loadSystem(); await loadModelComparison();
...
async function loadModelComparison() {
    try {
        ...
        if (data.length === 0) { tbody.innerHTML = '<tr><td colspan="6" class="p-4 text-center">No data.</td></tr>'; return; }
        tbody.innerHTML = '';
        data.forEach(row => {
            tbody.innerHTML += `<tr><td class="p-3">${row.Model}</td><td class="p-3">${row.Accuracy}%</td><td class="p-3 text-red-400">${row.Error_Rate}%</td><td class="p-3">${row.Precision}%</td><td class="p-3">${row.Recall}%</td><td class="p-3 text-purple-400">${row.F_Value}%</td></tr>`;
        });
...
async function loadSystem() {
    try {
        ...
        updateMap(data.chart_data); updateAIFeed(data);
...
async function submitCase() {
    ...
    try {
        const res = await fetch('/api/add-case', {
            ...
            body: JSON.stringify({ district: document.getElementById('entry-district').value, crime_type: document.getElementById('entry-type').value, description: document.getElementById('entry-desc').value, severity: "High" })
        });
        if(res.ok) { alert("✅ LOGGED"); document.getElementById('entry-desc').value = ''; await loadSystem(); await loadModelComparison(); }
    } catch(e) { alert("❌ Error"); }
...
function initMap() {
    ...
    legend.onAdd = function () {
        ...
        div.style.background = 'rgba(15,23,42,0.9)'; div.style.padding = '10px'; div.style.color = '#fff'; div.style.fontSize = '10px';
